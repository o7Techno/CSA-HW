...
struct Phone {
    pthread_mutex_t lock; // Мьютекс для телефона
        ...
        while (!action_chosen) {
            if (rand() % 2 == 0) {
                ...
                while (true) {
                    ...
                    if (pthread_mutex_trylock(&phones[id].lock) == 0) {
                        if (pthread_mutex_trylock(&phones[target].lock) == 0) {
                            if (!phones[target].is_busy && !phones[id].is_busy) {
                                ...
                                pthread_mutex_unlock(&phones[target].lock);
                                pthread_mutex_unlock(&phones[id].lock);
                                action_chosen = true;
                                break;
                            } else {
                                // Телефоны заняты, освободить мьютексы
                                pthread_mutex_unlock(&phones[target].lock);
                                pthread_mutex_unlock(&phones[id].lock);
                            }
                        } else {
...
int main(int argc, char* argv[]) {
    ...
    phones.resize(N);
    for (int i = 0; i < N; ++i) {
        pthread_mutex_init(&phones[i].lock, nullptr);
    ...
    // Очистка ресурсов
    for (int i = 0; i < N; ++i) {
        pthread_mutex_destroy(&phones[i].lock);
    }
